...
def print_xs(xs: DiffusionCrossSection):
    print(f"D  = np.array([{xs.D(0)}, {xs.D(1)}])")
    print(f"Es = np.array([[{xs.Es(0,0)}, {xs.Es(0,1)}],")
    print(f"               [{xs.Es(1,0)}, {xs.Es(1,1)}]])")
    print(f"Ea  = np.array([{xs.Ea(0)}, {xs.Ea(1)}])")
    print(f"vEf = np.array([{xs.vEf(0)}, {xs.vEf(1)}])")
    print( "Ef = vEf / 2.43")
    print( "chi = np.array([1., 0.])")
    print()
    print("xs = scb.DiffusionCrossSection(D, Ea, Es, Ef, vEf, chi, name='xs')")


ndl = NDLibrary("/home/hunter/Documents/nuclear_data/scarabee/endf8_scarabee125.h5")
...
asmbly.solve()
print_xs(asmbly.diffusion_data.xs)
...
refl.solve()
print_xs(refl.diffusion_data.xs)
